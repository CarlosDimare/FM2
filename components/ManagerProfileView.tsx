...
export const ManagerProfileView: React.FC<ManagerProfileViewProps> = ({ onBack }) => {
  ...
  if (!profile) return (
    <div className="flex flex-col h-full items-center justify-center" style={{ fontFamily: 'Verdana, sans-serif' }}>
      <p className="text-white/60">No hay perfil de entrenador disponible.</p>
    ...
    <div className="flex flex-col h-full overflow-hidden" style={{ fontFamily: 'Verdana, sans-serif' }}>
      <header className="bg-gradient-to-b from-[#e2e8f0] to-[#c8d2c8] p-4 border-b border-white/10 shrink-0">
        <div className="flex items-center gap-3 mb-3">
          <div className="w-14 h-14 rounded-full bg-white/25 border-2 border-white/10 flex items-center justify-center text-2xl">
            {p.origin === 'EX_PLAYER' ? '⚽' : p.origin === 'YOUTH_COACH' ? '📋' : '🎙️'}
          </div>
          <div>
            <h1 className="text-xl font-black text-white/90 uppercase tracking-wider italic">{p.fullName}</h1>
            <p className="text-[10px] text-white/50 uppercase tracking-[0.3em]">{ORIGIN_LABELS[p.origin]}</p>
            <div className="flex items-center gap-2 mt-1">
              <span className="text-xs text-white/70"><Flag size={12} className="inline mr-1" />{p.nationality}</span>
              <span className="text-[10px] text-white/40">|</span>
              <span className="text-xs text-white/70">{yearsOld} años</span>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-white/10 rounded-sm border border-white/10 p-3 text-center">
            <p className="text-2xl font-black text-white/90">{p.totalGames}</p>
            <p className="text-[9px] text-white/50 uppercase tracking-wider">Partidos</p>
          </div>
          <div className="bg-white/10 rounded-sm border border-white/10 p-3 text-center">
            <p className="text-2xl font-black text-green-600">{getWinRate(p)}%</p>
            <p className="text-[9px] text-white/50 uppercase tracking-wider">Victorias</p>
          </div>
          <div className="bg-white/10 rounded-sm border border-white/10 p-3 text-center">
            <p className="text-2xl font-black text-amber-600">{p.youthDebuts}</p>
            <p className="text-[9px] text-white/50 uppercase tracking-wider">Canteranos</p>
          </div>
        </div>

        <div className="bg-white/10 rounded-sm border border-white/10 p-4">
          ...
          <div className="space-y-2 text-xs">
            <div className="flex justify-between">
              <span className="text-white/50">Club actual</span>
              <span className="font-bold text-white/90">{p.currentClubName}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-white/50">Temporada en el club</span>
              <span className="font-bold text-white/90">{p.seasonInClub}ª</span>
            </div>
            <div className="flex justify-between">
              <span className="text-white/50">Balance</span>
              <span className="font-bold"><span className="text-green-600">{p.totalWins}</span> V · <span className="text-white/60">{p.totalDraws}</span> E · <span className="text-red-600">{p.totalLosses}</span> D</span>
            </div>
            <div className="flex justify-between">
              <span className="text-white/50">Goles</span>
              <span className="font-bold text-white/90">{p.goalsFor} GF · {p.goalsAgainst} GC</span>
            </div>
            {p.mostUsedPlayer !== 'Ninguno' && (
              <div className="flex justify-between">
                <span className="text-white/50">Jugador clave</span>
                <span className="font-bold text-white/90">{p.mostUsedPlayer}</span>
              </div>
            )}
            {p.biggestSale && (
              <div className="flex justify-between">
                <span className="text-white/50">Mayor venta</span>
                <span className="font-bold text-white/90">{p.biggestSale.player} (${(p.biggestSale.amount / 1000000).toFixed(1)}M)</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-white/10 rounded-sm border border-white/10 p-4">
          <h3 className="text-xs font-black uppercase tracking-wider text-white/70 mb-3 flex items-center gap-2">
            <Target size={14} /> Relaciones
          </h3>
          <div className="space-y-2 text-xs">
            <div className="flex justify-between items-center">
              <span className="text-white/50">Directiva</span>
            ...
            <div className="flex justify-between items-center">
              <span className="text-white/50">Prensa</span>
            ...
            <div className="flex justify-between items-center">
              <span className="text-white/50">Afición</span>
        ...
        <div className="bg-white/10 rounded-sm border border-white/10 p-4">
          ...
          <p className="text-xs text-white/80 font-bold">{p.currentObjective}</p>
        </div>
                ...
                return (
                  <div key={i} className="flex items-center justify-between text-xs border-b border-[#e0e8e0] pb-2 last:border-0">
                    <div className="flex items-center gap-2">
                      <div className={`w-3 h-3 rounded-full ${prevClub?.primaryColor || 'bg-white/50'} border border-white/10`}></div>
                      <span className="font-bold text-white/90">{entry.clubName}</span>
                    </div>
                    <span className="text-white/50">
                      {new Date(entry.startDate).getFullYear()}{entry.endDate ? ` - ${new Date(entry.endDate).getFullYear()}` : ' - Presente'}
                      <span className="ml-2 text-white/40">({entry.seasons} temp.)</span>
                    </span>
                  </div>
                );
              })}
