...
const StatCard = ({ icon, label, value, color }: { icon: React.ReactNode, label: string, value: string, color: string }) => (
  <div className="bg-white/10/10 border border-white/10 rounded-sm shadow-sm flex items-center gap-4 p-4 group hover:border-[#3a4a3a] transition-all">
    <div className="p-3 bg-gradient-to-b from-[#f0f4f0] to-[#d0d8d0] border border-white/10 rounded-sm group-hover:scale-105 transition-transform shadow-inner">
      {icon}
    </div>
    <div className="min-w-0 flex-1">
      <p className="text-[9px] font-black text-white/50 uppercase tracking-widest truncate mb-1" style={{ fontFamily: 'Verdana, sans-serif' }}>{label}</p>
...
const FinanceRow = ({ label, value, type }: { label: string, value: number, type: 'INCOME' | 'EXPENSE' }) => (
  <div className="flex justify-between items-center py-2.5 border-b border-white/10 last:border-0 hover:bg-white/10 px-2 transition-colors">
    <span className="text-white/60 font-bold uppercase text-[10px] tracking-wide" style={{ fontFamily: 'Verdana, sans-serif' }}>{label}</span>
    <div className="flex items-center gap-2">
      {type === 'INCOME' ? <TrendingUp size={14} className="text-green-600" /> : <TrendingDown size={14} className="text-red-600" />}
      <span className={`font-mono font-bold text-sm ${type === 'INCOME' ? 'text-green-400' : 'text-white/70'}`}>
...
export const EconomyView: React.FC<EconomyViewProps> = ({ club }) => {
  ...
  return (
    <div className="p-2 md:p-4 flex flex-col gap-4 h-full overflow-hidden">
      <header className="shrink-0 flex flex-col gap-1">
        ...
        <p className="text-white/60 text-[10px] font-bold uppercase tracking-widest italic" style={{ fontFamily: 'Verdana, sans-serif' }}>Balance financiero y presupuestos de gestión.</p>
      </header>

      {/* Top Stats Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 shrink-0">
        <StatCard 
          icon={<Landmark className="text-white/70" size={24} />} 
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-4 min-h-0 overflow-y-auto lg:overflow-hidden pb-4 lg:pb-0">
        ...
        <FMBox 
          ...
          headerRight={
            <span className={`text-[9px] font-black px-2 py-0.5 rounded-sm border ${netProfit >= 0 ? 'bg-green-500/15 border-green-300 text-green-300' : 'bg-red-100 border-red-300 text-red-800'}`} style={{ fontFamily: 'Verdana, sans-serif' }}>
              {netProfit >= 0 ? '+' : ''}{netProfit.toLocaleString()} / mes
            </span>
          }
        >
          <div className="p-2 space-y-1 bg-white/10 flex-1 overflow-y-auto custom-scroll">
            ...
            <div className="px-2 py-1 mt-2 text-[8px] font-black text-red-400 uppercase tracking-widest border-b border-red-200">Gastos</div>
            ...
            <div className="mt-8 px-2 py-6 bg-white/10/5 border-t border-white/10 rounded-b-sm flex justify-between items-center shadow-inner">
              <span className="font-black text-white/50 text-[11px] uppercase tracking-tighter" style={{ fontFamily: 'Verdana, sans-serif' }}>Balance Mensual Neto</span>
              <span className={`text-3xl font-black italic tracking-tighter ${netProfit >= 0 ? 'text-green-400' : 'text-red-400'}`} style={{ fontFamily: 'Verdana, sans-serif' }}>
        ...
        <FMBox title="Salud Financiera" className="h-full flex flex-col justify-center shadow-lg">
          <div className="flex flex-col items-center text-center py-6 h-full justify-between">
            <div className="relative group mb-4">
              <div className="p-6 bg-gradient-to-b from-[#f0f4f0] to-[#d0d8d0] rounded-full shadow-xl border-2 border-white/10 group-hover:scale-105 transition-transform duration-500">
                <PiggyBank size={48} className="text-[#1a2a1a] drop-shadow-sm" />
              </div>
              <div className="absolute -bottom-2 -right-2 bg-white/25 text-white text-[8px] font-black px-2 py-1 rounded shadow-lg border border-white/20">ESTADO</div>
            </div>
            ...
            <div className="max-w-md px-4 mb-6">
              ...
              <p className="text-white/70 text-xs font-bold uppercase leading-relaxed tracking-tight" style={{ fontFamily: 'Verdana, sans-serif' }}>
            ...
            <div className="w-full bg-white/10 p-4 rounded-sm border border-white/10 shadow-inner space-y-2">
               <div className="flex justify-between text-[10px] font-black uppercase text-white/50 tracking-tighter" style={{ fontFamily: 'Verdana, sans-serif' }}>
               ...
               <div className="w-full bg-white/10/10 h-4 rounded-[1px] overflow-hidden border border-white/10 relative">
                  <div 
               ...
               <div className="flex justify-between items-center pt-1">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[8px] text-white/40 font-bold uppercase">Presupuesto Limite: £{wageBudget.toLocaleString()}</span>
                  </div>
                  <span className="text-[9px] font-black text-white/70 uppercase">{currentSalaries.toLocaleString()} / £{wageBudget.toLocaleString()}</span>
               </div>
            </div>
           </div>
        </FMBox>
          ...
          <FMBox title="Historial Mensual" noPadding className="mt-4">
            <FMTable headers={['Mes', 'Ingresos', 'Egresos', 'Neto', 'Balance']} colWidths={['80px', 'auto', 'auto', 'auto', 'auto']}>
              {[...club.finances.monthlyHistory].reverse().map((entry, i) => (
                <tr key={i} className={`${i % 2 === 0 ? 'bg-white' : 'bg-white/5'} hover:bg-white/10 transition-colors`}>
                  <FMTableCell className="font-bold text-white/70 text-[10px]">{entry.month}</FMTableCell>
                  <FMTableCell className="text-green-400 text-[10px]" isNumber>£{entry.income.toLocaleString()}</FMTableCell>
                  <FMTableCell className="text-red-400 text-[10px]" isNumber>£{entry.expenses.toLocaleString()}</FMTableCell>
                  <FMTableCell className={`font-black text-[10px] ${entry.income - entry.expenses >= 0 ? 'text-green-400' : 'text-red-400'}`} isNumber>
                    {entry.income - entry.expenses >= 0 ? '+' : ''}£{(entry.income - entry.expenses).toLocaleString()}
                  </FMTableCell>
                  <FMTableCell className="font-bold text-white/90 text-[10px]" isNumber>£{entry.balance.toLocaleString()}</FMTableCell>
                </tr>
              ))}
            </FMTable>
          </FMBox>
