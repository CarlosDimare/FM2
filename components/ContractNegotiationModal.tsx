...
export const ContractNegotiationModal: React.FC<ContractNegotiationModalProps> = ({ player, userClubId, currentDate, onClose, onRenewed }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/90 z-[200] flex items-center justify-center p-4 backdrop-blur-xl">
      <div className="bg-[#2a3a2a] w-full max-w-lg rounded-3xl shadow-2xl border border-[#1a2a1a] overflow-hidden flex flex-col">
        ...
        <div className="p-8 space-y-8">
           ...
           <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-900/50 p-4 rounded-xl border border-[#3a4a3a]">
           ...
           <div className="space-y-6">
              ...
              <div className="space-y-2">
                 <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Duración del Vínculo</label>                  <div className="flex bg-slate-900 p-1 rounded-xl border border-[#3a4a3a]">
                    {[1, 2, 3, 4, 5].map(y => (
                       <button 
                          key={y}
                          disabled={isLocked}
                          onClick={() => setYears(y)}
                          className={`flex-1 py-3 text-xs font-black rounded-lg transition-all ${years === y ? 'bg-[#3a4a3a] text-white shadow-lg' : 'text-slate-500 hover:text-white'}`}
                       >
                          {y} {y === 1 ? 'AÑO' : 'AÑOS'}
                       </button>
                    ))}
           ...
           <div className="flex flex-col gap-3">
              <button 
                 ...
                 disabled={isLocked}
                 className={`w-full py-5 rounded-2xl font-black uppercase tracking-[0.2em] shadow-xl transition-all active:scale-95 flex items-center justify-center gap-3 ${isLocked ? 'bg-slate-700 text-slate-500 cursor-not-allowed' : 'bg-[#4a6a4a] hover:bg-[#5a7a5a] text-white'}`}
              >
        ...
        <div className="bg-slate-900 p-4 border-t border-[#3a4a3a] flex items-center gap-4">
           <div className="w-10 h-10 rounded-full bg-[#3a4a3a] flex items-center justify-center text-amber-400 shrink-0">
              <AlertCircle size={20} />
           </div>
