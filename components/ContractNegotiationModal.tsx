...
export const ContractNegotiationModal: React.FC<ContractNegotiationModalProps> = ({ player, userClubId, currentDate, onClose, onRenewed }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/90 z-[200] flex items-center justify-center p-4 backdrop-blur-xl">
      <div className="bg-white/10/10 w-full max-w-lg rounded-3xl shadow-2xl border border-slate-700 overflow-hidden flex flex-col">
        <header className="p-6 bg-slate-900 border-b border-slate-700 flex justify-between items-center">
           ...
           <button onClick={onClose} className="text-white/50 hover:text-white" disabled={isLocked}>
              <X size={28} />
           </button>
        </header>
        ...
        <div className="p-8 space-y-8">
           ...
           <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-700">
                 <p className="text-[9px] font-black text-white/50 uppercase tracking-widest mb-1">Sueldo Actual</p>
           ...
           <div className="space-y-6">
              <div className="space-y-2">
                 <label className="text-[10px] font-black text-white/40 uppercase tracking-widest block">Propuesta Salarial</label>
                 ...
                 <div className="flex justify-between px-1">
                    <span className="text-[10px] text-white/50 font-bold uppercase">Paciencia: {3 - player.negotiationAttempts}/3 intentos</span>
              ...
              <div className="space-y-2">
                 <label className="text-[10px] font-black text-white/40 uppercase tracking-widest block">Duración del Vínculo</label>
                 <div className="flex bg-slate-900 p-1 rounded-xl border border-slate-700">
                    {[1, 2, 3, 4, 5].map(y => (
                       <button 
                          ...
                          disabled={isLocked}
                          onClick={() => setYears(y)}
                          className={`flex-1 py-3 text-xs font-black rounded-lg transition-all ${years === y ? 'bg-blue-600 text-white shadow-lg' : 'text-white/50 hover:text-white'}`}
                       >
                          {y} {y === 1 ? 'AÑO' : 'AÑOS'}
                       </button>
                    ))}
           ...
           <div className="flex flex-col gap-3">
              <button 
                 ...
                 disabled={isLocked}
                 className={`w-full py-5 rounded-2xl font-black uppercase tracking-[0.2em] shadow-xl transition-all active:scale-95 flex items-center justify-center gap-3 ${isLocked ? 'bg-slate-700 text-white/50 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-500/100 text-white'}`}
              >
              ...
              <button 
                 onClick={onClose}
                 disabled={isLocked}
                 className="w-full py-3 text-white/50 hover:text-red-400 transition-colors text-[10px] font-black tracking-widest uppercase"
              >
        ...
        <div className="bg-slate-900 p-4 border-t border-slate-700 flex items-center gap-4">
           <div className="w-10 h-10 rounded-full bg-white/10/10 flex items-center justify-center text-blue-400 shrink-0">
              <AlertCircle size={20} />
           </div>
