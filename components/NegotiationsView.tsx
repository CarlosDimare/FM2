...
export const NegotiationsView: React.FC<NegotiationsViewProps> = ({ userClubId, currentDate }) => {
  ...
  const getStatusStyle = (status: string) => {
    switch (status) {
      case 'ACCEPTED': return 'bg-green-500/15 text-green-300 border-green-300';
  ...
  const renderOfferRow = (offer: TransferOffer) => {
    ...
    return (
      <div key={offer.id} className="bg-white/10/10 border border-white/10 rounded-sm p-4 shadow-sm hover:border-[#3a4a3a] transition-all group mb-2">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex items-center gap-4 w-full md:w-auto">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center font-black text-white shrink-0 shadow-inner ${isBuying ? 'bg-white/10' : 'bg-[#aabdaa]'}`}>
              {isBuying ? <ArrowRightLeft size={20} /> : <DollarSign size={20} />}
            </div>
            <div className="min-w-0">
              <h4 className="text-white/90 font-black text-sm uppercase italic truncate">{player?.name || 'Jugador desconocido'}</h4>
              <p className="text-white/50 text-[10px] font-bold uppercase tracking-tight truncate">
          ...
          <div className="flex-1 grid grid-cols-2 md:grid-cols-3 gap-4 w-full md:w-auto">
            <div className="bg-white/10/5 p-2 rounded border border-slate-100">
               <p className="text-[8px] uppercase font-black text-white/40">Tipo de Trato</p>
               <p className="text-[10px] font-bold text-white/80 uppercase">{offer.type === 'PURCHASE' ? 'Traspaso' : 'Cesión'}</p>
            </div>
            
            <div className="bg-white/10/5 p-2 rounded border border-slate-100">
               <p className="text-[8px] uppercase font-black text-white/40">{offer.type === 'PURCHASE' ? 'Costo' : 'Sueldo asumido'}</p>
          ...
          <div className="flex items-center gap-2 w-full md:w-auto shrink-0">
            ...
            {offer.status === 'PENDING' && (
              <span className="text-[9px] text-white/50 italic font-bold">Esperando respuesta del club...</span>
            )}
          </div>
        </div>
        
        <div className="mt-3 pt-3 border-t border-slate-50 flex justify-between items-center">
           <div className="text-[9px] text-white/40 font-bold uppercase flex items-center gap-1.5">
  ...
  return (
    <div className="p-2 md:p-4 h-full flex flex-col gap-4 overflow-hidden">
      <header className="flex flex-col gap-3 shrink-0">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h2 className="text-xl md:text-2xl font-black text-white/90 uppercase italic tracking-tighter leading-none">Centro de Fichajes</h2>
            <p className="text-white/60 font-bold text-[9px] md:text-[10px] uppercase tracking-widest italic mt-1">Acuerdos, cesiones y confirmaciones de contrato.</p>
          </div>
          
          <div className="flex bg-white/10/10 p-0.5 rounded-sm border border-white/10 shadow-sm w-full md:w-auto">
             <button 
                onClick={() => setActiveTab('ACTIVE')}
                className={`flex-1 md:px-6 py-2 text-[9px] font-black rounded-[1px] transition-all uppercase tracking-widest flex items-center justify-center gap-2 ${activeTab === 'ACTIVE' ? 'bg-white/25 text-white shadow-sm' : 'text-white/70 hover:bg-white/10'}`}
             >
                <MessageSquare size={14} /> ACTIVAS ({activeOffers.length})
             </button>
             <button 
                onClick={() => setActiveTab('HISTORY')}
                className={`flex-1 md:px-6 py-2 text-[9px] font-black rounded-[1px] transition-all uppercase tracking-widest flex items-center justify-center gap-2 ${activeTab === 'HISTORY' ? 'bg-white/25 text-white shadow-sm' : 'text-white/70 hover:bg-white/10'}`}
             >
         ...
         <div className="flex flex-col">
            {activeTab === 'ACTIVE' ? (
               activeOffers.length === 0 ? (
                  <div className="p-20 text-center text-white/40 italic flex flex-col items-center justify-center gap-3 bg-white/10/10 rounded-sm border border-dashed border-white/10">
      ...
      <div className="bg-white/10/10/50 p-4 rounded-sm border border-white/10 flex items-start gap-4 shrink-0 shadow-inner">
         <div className="p-2 bg-white/10/10 rounded-full text-white/70 border border-white/20">
            <Info size={16} />
         </div>
         <div className="flex-1">
            <p className="text-[10px] text-white/80 font-black uppercase tracking-tight">Manual de Negociación</p>
            <p className="text-[9px] text-white/60 font-bold uppercase leading-tight mt-1">
               Un trato aceptado por el club rival NO incorpora al jugador automáticamente. Debes pulsar <span className="text-green-400">"FIRMAR JUGADOR"</span> para cerrar el contrato personal y que se una a tu disciplina.
            </p>
         </div>
      </div>
    </div>
